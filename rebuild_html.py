...
function drawEmb(){
 emb.clearRect(0,0,540,200); const pad=25,W=540-2*pad,H=200-2*pad;
 ...
 emb.beginPath();emb.arc(tx(D.proj[k][0]),ty(D.proj[k][1]),9,0,7);emb.lineWidth=2.5;emb.strokeStyle='#780010';emb.stroke();
}
...
function drawDrift(){
 ...
 drift.strokeStyle='#2E75B6';drift.lineWidth=1.8;drift.beginPath();
 ...
 for(let i=1;i<=k;i++){
  if(D.labels[i]!=D.labels[i-1]){
   drift.strokeStyle='rgba(120,0,16,0.65)';drift.setLineDash([4,4]);
...
function drawTL(){
 ...
 const cx=pad+k/(N-1)*W;tl.strokeStyle='#780010';tl.lineWidth=2.5;tl.beginPath();tl.moveTo(cx,y-10);tl.lineTo(cx,y+h+10);tl.stroke();
 tl.textAlign='left';
}

function drawLegend(){
 const leg=document.getElementById('legend'); if(!leg)return;
 leg.innerHTML='<span style="font-weight:700;color:#780010;margin-right:8px;">Phase Legend:</span>'+
  Object.keys(PN).map(key=>`<span style="display:inline-flex;align-items:center;gap:4px;margin-right:12px;font-size:11.5px;font-weight:600;"><span style="width:12px;height:12px;border-radius:3px;background:${PC[key]};display:inline-block;"></span><span style="color:#262626;">${PN[key]}</span></span>`).join('');
}
